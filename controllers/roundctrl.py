...
class RoundController:
    ...
    @classmethod
    def round_one_matches(cls, my_tournament, my_players):
        ...
        # 1er round avec round créé
            ...
            matches = MatchController.instantiate_round_matchs_from_json(
                my_tournament.rounds_tour[0]['matches'], my_players)
            round1 = Round("Round 1",
                           datetime.today().strftime('%d/%m/%Y %H:%M:%S'),
                           None, matches)
            RoundView.roundprint(my_tournament, round1.to_dict_results())
    ...
    def round_get_results(cls, my_tournament, _round):
        """permet à l'utilisateur de choisir entre :
            - inscription des résultats du round de suite,
            - inscription des résultats plus tard
        et de les retourner dans tous les cas.
        Args:
        ...
        if return_results is not None and return_results != 'None':
    ...
    def round_results(cls, _round, return_results):
        ...
        pour finaliser les infos d'un round que l'utilisateur a choisi de
        renseigner
        Args:
        ...
        # iteration avec index pour faire correspondre résultats inscrits
        # aux matchs :
        for index, _match in enumerate(matches_list):
            if _match.player_id2 is not None:
                ...
                _match.score_p1, _match.score_p2 = _match.set_result(
                        winner)
        _round.end_date = datetime.today().strftime('%d/%m/%Y %H:%M:%S')
        return _round

    @classmethod
    def make_next_round(cls, my_tournament, my_players):
        """permet de créer les rounds (n+1) avec classement par points
        Args:
            mytournament (Tournament): Instance du tournoi en cours.
            my_players (list of Player): Liste des joueurs
            instanciés prêts à concourir.
        Returns:
