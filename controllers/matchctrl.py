...
class MatchController:
    ...
    @classmethod
    def instantiate_round_matchs_from_json(cls, round_matches, instantiated_players):
        ...
        for _match in round_matches:
            for player in instantiated_players:
                ...
                if player.chess_id==_match[1][0]:
                    player2=player
            match_instance = MatchModel(player1, _match[0][1], player2, _match[1][1])
    ...
    def make_next_round_matchs(cls, instantiated_players, my_tournament):
        ...
        selon les points (classement) et joueurs précédemment rencontrés (à éviter au maximum)
        """
        sorted_players = sorted(instantiated_players, key=lambda d: d.points, reverse=True)
        next_round_matches = []
        played_against = MatchController.get_played_against(my_tournament)
        for player in sorted_players:
            player_id = player.chess_id
            if player_id not in played_against:
                played_against[player_id] = set()
            # Trouver un adversaire non joué "opponent" pour le joueur actuel "player"
            opponent = None
            for potential_opponent in sorted_players:
                potential_opponent_id = potential_opponent.chess_id
                if potential_opponent_id != player_id and potential_opponent_id not in played_against[player_id]:
                    opponent = potential_opponent
                    break
            # Si un adversaire est trouvé, on crée le match et on enlève ces joueurs de la liste
            if opponent:
                next_round_matches.append(MatchModel(player,None,opponent,None))
                sorted_players.remove(player)
                sorted_players.remove(opponent)
            else:
                # aucun nouvel adversaire valide n'est trouvé,
                # s'il est tout seul=>match factice
                if len(sorted_players) == 1:
                    next_round_matches.append(MatchModel(player,"None",None,0))
                    sorted_players.remove(player)
                # sinon rejouer un match existant.
                else:
                    next_round_matches.append(MatchModel(player,"None",sorted_player[1],"None"))
                    sorted_players.remove(player)
    ...
    def get_played_against(cls, my_tournament):
        ...
        """
        played_against = {}
        # Parcourir tous les tours terminés pour remplir le dictionnaire "played_against"
        for _round in my_tournament.rounds_tour:
            for match in _round['matches']:
                player1, player2 = match[0][0], match[1][0]  # Assuming a match is structured as [[player1, score], [player2, score]]
